...
def test_generate_quizzes_endpoint():
    ...
    with patch("routers.workspace.generate_quizzes_from_job_description", return_value=dummy_quizzes):
        response = client.post(f"/workspaces/{workspace.id}/quizzes/generate")
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)
        assert len(data) == len(dummy_quizzes)
        
        # Verify response (without correct_answer)
        for returned, expected in zip(data, dummy_quizzes):
        ...
        assert len(db_quizzes) == len(dummy_quizzes)
        for db_q, expected in zip(db_quizzes, dummy_quizzes):
            assert db_q.question == expected["question"]
            assert db_q.options == expected["options"]
            assert db_q.correct_answer == expected["correct_answer"]
        db.close()
